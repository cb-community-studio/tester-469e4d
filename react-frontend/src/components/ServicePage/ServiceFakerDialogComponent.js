...
import serviceFakerFactory from "./serviceFakerFactory";
...
const ServiceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = serviceFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("service").create(elem));
...
export default connect(null, mapDispatch)(ServiceFakerDialogComponent);
