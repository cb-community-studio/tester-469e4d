...
import test2FakerFactory from "./test2FakerFactory";
...
const Test2FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = test2FakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("test2").create(elem));
...
export default connect(null, mapDispatch)(Test2FakerDialogComponent);
