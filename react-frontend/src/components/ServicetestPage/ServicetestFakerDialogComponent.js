...
import servicetestFakerFactory from "./servicetestFakerFactory";
...
const ServicetestFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = servicetestFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("servicetest").create(elem));
...
export default connect(null, mapDispatch)(ServicetestFakerDialogComponent);
