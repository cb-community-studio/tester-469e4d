...
import test1FakerFactory from "./test1FakerFactory";
...
const Test1FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = test1FakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("test1").create(elem));
...
export default connect(null, mapDispatch)(Test1FakerDialogComponent);
