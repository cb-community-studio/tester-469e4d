...
import itemsFakerFactory from "./itemsFakerFactory";
...
const ItemsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = itemsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("items").create(elem));
...
export default connect(null, mapDispatch)(ItemsFakerDialogComponent);
