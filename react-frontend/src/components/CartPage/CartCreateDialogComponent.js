...
const CartCreateDialogComponent = (props) => {
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="cart-create-dialog-component">
                <div>
                    <p className="m-0" >Cart ID:</p>
                    <InputText className="w-full mb-3" value={_entity?.CartID} onChange={(e) => setValByKey("CartID", e.target.value)}  />
                </div>
                <div>
...
export default connect(null, mapDispatch)(CartCreateDialogComponent);
