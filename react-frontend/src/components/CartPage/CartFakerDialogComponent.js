...
import cartFakerFactory from "./cartFakerFactory";
...
const CartFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cartFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cart").create(elem));
...
export default connect(null, mapDispatch)(CartFakerDialogComponent);
